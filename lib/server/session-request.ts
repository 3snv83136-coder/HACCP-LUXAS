...
import { sessionLibre } from "@/lib/server/acces-libre";

export async function sessionDepuisCookie(): Promise<SessionPayload | null> {
  return (await readSession(cookies().get(SESSION_COOKIE)?.value)) ?? sessionLibre();
}
...
export async function sessionDepuisRequete(request: Request): Promise<SessionPayload | null> {
  ...
  if (part) {
    const session = await readSession(decodeURIComponent(part.slice(SESSION_COOKIE.length + 1)));
