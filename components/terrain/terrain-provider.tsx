...
export function TerrainProvider({ children }: { children: ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="flex min-h-dvh items-center justify-center text-white/60">Chargement…</div>
    );
  }
    ...
    return (
      <div className="mx-auto flex min-h-dvh max-w-md flex-col justify-between px-5 py-8">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">Ton code</h1>
          <p className="mt-2 text-sm text-white/55">
            Signature personnelle. Chaque relevé restera horodaté à ton nom.
          </p>
          <p className="mt-8 text-center font-mono text-5xl tracking-[0.4em] text-white">
            {pin.padEnd(4, "•")}
          </p>
        ...
        <div className="space-y-4">
          ...
          <p className="text-center text-xs text-white/35">Démo : 2580 opérateur · 1470 responsable</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <TerrainContext.Provider value={value}>
      <div className="mx-auto flex min-h-dvh max-w-lg flex-col">
        <header className="sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-white/10 bg-[#07111a]/90 px-4 py-3 backdrop-blur">
          <div>
            <p className="text-[11px] uppercase tracking-[0.18em] text-teal-300">Sanitrace</p>
          ...
          <div className="flex items-center gap-2">
            <SyncPill />
            <button type="button" onClick={() => void signOut()} className="text-xs text-white/40">
              Quitter
            </button>
          </div>
        </header>
